...
| [T-008](#t-008-前端构建链确认-nextjs-vs-vite) | 前端构建链确认（Next.js vs Vite） | 已定 ✅ | A. Next.js (App Router) + shadcn/ui |
...
**状态**：已定 ✅（2026-05-01）。结论：**A. Next.js (App Router) + shadcn/ui**。
...
3. **T-001**（数据库）— 影响 T-002、T-003、T-004 ⬅️ 下一个推进
4. **T-002**（ORM）— 跟随 T-001
5. **T-005**（部署）— 影响 T-004
6. **T-004**（图片存储）— 跟随 T-005
7. **T-003**（鉴权）— 较独立
8. **T-006**（编辑器）— 较独立
9. **T-007**（域名）— 上线前再敲
